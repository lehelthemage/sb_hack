...
void sys_glDrawElements (GLenum mode,  GLsizei count,  GLenum type,  const GLvoid *indices)
{
	...
	//First get your model recognition. It can be counts, crc, or return address.
//then this is the code for flashing guns and models
//if(overlay_on)
//{
//
//	orig_glEnable(GL_COLOR_MATERIAL);  //Enables coloring
//	orig_glColor3f(rand()%2,rand()%2, rand()%2); //This will choose a random color each time DrawElements is called
//	orig_glDisable(GL_COLOR_MATERIAL);  //Disables Coloring
//
////This is a regular colored model
//
//	orig_glEnable(GL_COLOR_MATERIAL);  //Enables Coloring
//	orig_glColor3f(1.0f,0.0f,0.0f);  //Makes it Red. You can change it to any color you want. It goes RGB
//	orig_glDisable(GL_COLOR_MATERIAL); //Disables Coloring
//
////This is chams
//
...
//	orig_glEnable(GL_COLOR_MATERIAL);  //Enables Coloring
//	orig_glColor3f(0.0f,1.0f,0.0f);  //Makes it Green. You can change it to any color you want. It goes RGB
//	orig_glDisable(GL_COLOR_MATERIAL); //Disables Coloring
//}
	(*orig_glDrawElements) (mode, count, type, indices); //Redraws textures
//if(overlay_on)
//{
...
//	orig_glDisable(GL_COLOR_MATERIAL); //Disables Coloring
//
////This is Enlarging the models, for a glow effect
//
//	orig_glEnable(GL_DEPTH_TEST);  //Makes it a wallhack model
//	orig_glEnable(GL_BLEND);  //Enables Blending
//	orig_glBlendFunc(GL_ONE_MINUS_DST_COLOR,GL_DST_ALPHA);  //The Blending type
//	orig_glEnable(GL_COLOR_MATERIAL);  //Enables Coloring
//	orig_glColor3f(1.0f,0.0f,0.0f);  //Makes it a red ghost like color
//	orig_glTranslatef(2.0,2.0,2.0);  //Makes the model 2 times as big, but also red and transparent
...
BOOL Init (void)
{
	if ( !hOriginalDll )
	{
		...
		hOriginalDll = LoadLibraryA(sysdir);
